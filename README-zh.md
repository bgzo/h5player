...
- 位置：菜单「更多操作」→「启用/禁用 跨CORS截图」（默认关闭）
- 行为：截图因视频源禁止 CORS 导出而失败时，重新拉取视频源、将当前帧重绘到干净 canvas 后下载
- 适用性：仅对直接 mp4/webm 视频文件生效，HLS / MSE / DASH（.m3u8）源不支持
- 流量警告：开启后需重新下载整段视频，可能消耗大量流量；同一视频源仅下载一次并缓存复用
- 缓存规则：大于 1GB 或无法探测大小的视频不缓存；超过 4 小时无截图自动释放缓存
- 联网范围：该功能依赖用户脚本的 `@connect *` 授权，开启时可向视频源所在域名发起跨域请求
- 注意：重拉视频源所用的 `Referer` 与 `Range` 请求头在部分用户脚本宿主上可能被静默丢弃（例如某些浏览器下的 Violentmonkey），此类宿主上该功能可能无法绕过 CORS
- 凭据：重拉请求默认不携带 Cookie（菜单项"跨CORS截图携带凭据"默认关闭）。公开 CDN 应保持关闭；需要登录鉴权的源可在菜单中开启。
